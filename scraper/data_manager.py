...
class Data_manager:
    def __init__(self, json_path = settings.raw_json_dir):
    ...
    def save_normal_json(self, file_name, params):
        ...
        with open(file_path, "w", encoding='utf-8') as f:
            json.dump(params, f, ensure_ascii=False, indent=4)
    def check_file_exist(self, date_str):
        file_path = self.json_path / f"{date_str[:4]}-{date_str[5:7]}-{date_str[8:]}.json"
        return file_path.is_file()

def process_single_day(date, data_manager, hook):
    if data_manager.check_file_exist(date):
        print(f"[跳過] 日期 {date} 已存在本地 JSON 檔案。")
        return
    try:
        data_manager.check_file_exist(date)
        ...
        data_manager.save_races_json(date,races_params)
        print("Saved to json")
    except:
        print("Error occured")
...
def process_rating():
    ...
    data_manager = Data_manager(json_path=settings.rating_path)
